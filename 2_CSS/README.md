...
- flex-direction과 flex-wrap을 한꺼번에 지정 할 수 있는 속성
...
```css
.container {
  justify-content: flex-start / flex-end / center /*많이*/ / space-between
    /*많이*/ / space-around/ space-evenly;
}
```
...
```css
.item {
  flex-shrink: 1 /*기본값*/ / 숫자;
}
```

## flex

- flex-grow, flex-shrink, flex-basis를 한번에 쓸 수 있느 속성

```css
.item {
  flex: 1 1 auto;
  /*flex -grow: 1; flex-shrink: 1; flex-basis: auto; 와 같은 뜻*/

  flex: 1 500px;
  /*flex -grow: 1; flex-shrink: 1; flex-basis: 500px; 와 같은 뜻*/
  flex: 1;
  /*flex -grow: 1; flex-shrink: 1; flex-basis: 0%; 와 같은 뜻*/
}
```
