...
$(function () {
  ...
  // 2. 아이디 선택자
  // 자바스크립트 방식 -> id가 id1일 글씨 색깔(color)변경 (색상은 자유롭게)
  ...
  $(".item").css({ color: "skyblue", "background-color": "indigo" });

  // li-> color : green
  $("ul > li").css("color", "green");
  //<!-- 클래스가 ch -> background-color: lime   -->
  $("ul > .ch").css("background-color", "lime");
  //h3 -> color : indianred
  $("h3").css("color", "indianred");
  //h3이면서 클래스가 ch -> color: violet
  $("div > .ch").css("color", "violet");
  //h3공통 : background-color: khaki
